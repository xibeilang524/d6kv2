...
Version: 1.0.0.0
Date: 2014.5.19
Description: 达梦DM接口头文件，数据结构体定义
Others: 
Function List: DBComm
History: 
*************************************************/
#if !defined( AFX_DBAPI_H__DBAPI_DMSQL_H )
#define AFX_DBAPI_H__DBAPI_DMSQL_H
...
class DBComm : public NetDbg 
{
	...
	static OCIEnv* m_envhp;				//环境句柄
	static OCIError* m_errhp;			//错误句柄
	OCIServer* m_srvhp;					//服务器句柄
	OCISvcCtx* m_svchp;					//服务句柄，也可以说是连接句柄
	OCISession* m_authp;					//会话句柄
	OCIStmt* m_stmthp;					//语句描述句柄
	static dyn_ctx_struct  *dynamic_context;
	static short		m_obj;
	static LOG_INFO*	logp;					//用户登录信息
