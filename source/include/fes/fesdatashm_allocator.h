...
public:
	...
	*/
	...
	@brief 初始化通道的参数
	*/
	void inital_para(int channel_no);
...
public:
	...
	*/
	FE_YC* set_yc_value(const int& channel, const int& data_no, const float& value,
	...
	最后把生成的FE_DATA_MSG_NODE写入全局缓存（这个暂时不用实现）
	*/
	FE_YX* set_yx_value(const int& channel, const int& data_no, const unsigned char& value,
		const unsigned char& quality, FETIME* time = NULL);
	/**
	@brief 设置电度值
	...
	最后把生成的FE_DATA_MSG_NODE写入全局缓存（这个暂时不用实现）
	*/
	FE_KWH* set_kwh_value(const int& channel, const int& data_no, const unsigned long& value,
		const unsigned char& quality, FETIME* time = NULL);
...
private:
	...
	*/
	...
	unsigned short yc_block_size;
	unsigned short yx_block_size;
	unsigned short kwh_block_size;
