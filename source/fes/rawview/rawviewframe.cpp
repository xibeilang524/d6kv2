...
QRawViewFrame::QRawViewFrame( QWidget* parent /* = NULL */ ) :QFrame( parent )
{
	...
	m_pTabWidget->addTab( m_pYcTable, _C2Q( "遥测" ) );
	m_pTabWidget->addTab( m_pYxTable, _C2Q( "遥信" ) );
	m_pTabWidget->addTab( m_pKwhTable, _C2Q( "电度" ) );
...
void QRawViewFrame::initTable(  )		//加入变化数据时标显示
{
...
// 	uint  kwhValue;
	...
	for ( i = 0 ; i < fesshm.rtu_para_ptr[ m_curRTU ].yc_num ; i++ )
	{
		...
		{
			m_pYcTable->setItem( i, 3, new QTableWidgetItem( _C2Q( "无效" ) ) );
		}
		else 
		{
			m_pYcTable->setItem( i, 3, new QTableWidgetItem( _C2Q( "有效" ) ) );
		}
...
// 		m_pYcTable->setItem( i, 4, new QTableWidgetItem( QString::number( mktime( &procsec ) ) ) );
	...
	for ( i = 0 ; i < fesshm.rtu_para_ptr[ m_curRTU ].yx_num ; i++ )
	{
		...
		{
			m_pYxTable->setItem( i, 2,  new QTableWidgetItem( _C2Q( "分" ) ) );
		}

		if ( UCHAR_DEFAULT == yxValue )
		{
			m_pYxTable->setItem( i, 4, new QTableWidgetItem( _C2Q( "无效" ) ) );
		}
		else 
		{
			m_pYxTable->setItem( i, 4, new QTableWidgetItem( _C2Q( "有效" ) ) );
		}
		yxValid = yx_ptr->modifier.quality ;
		if(yxValid)
		{
			m_pYxTable->setItem( i, 5, new QTableWidgetItem( _C2Q( "有效" ) ) );
		}
		else
		{
			m_pYxTable->setItem( i, 5, new QTableWidgetItem( _C2Q( "无效" ) ) );
		}
...
// 		m_pYxTable->setItem( i, 6, new QTableWidgetItem( QString::number( mktime( &procsec ) ) ) );
	...
	for ( i = 0 ; i < fesshm.rtu_para_ptr[ m_curRTU ].kwh_num ; i++ )
	{
		...
		{
			m_pKwhTable->setItem( i, 3, new QTableWidgetItem( _C2Q( "无效" ) ) );
		}
		else 
		{
			m_pKwhTable->setItem( i, 3, new QTableWidgetItem( _C2Q( "有效" ) ) );
		}
