...
int Iec101b::Iec101b_send_proc(void)
{
	...
	else if ( m_link_flag != REQUESTLINKSTATUS && m_link_flag != RESET )
	{
        m_link_flag = -1;
		m_hearttext_cnt++;
	}
	/*else if ( m_pi_all_num && (m_call_kwh_data_flag == NEED_CALL) && m_call_all_data_flag != ON_PROCESS)	//若有电度量且需要召唤 总召过程中不进行电度召唤
	{
			ret = call_kwh_data();
			m_t0_timeout = true ;
	}*/
...
int Iec101b::call_classone_data( void )
{
	m_substation_ACD = 0 ;
	ACE_OS::memset(m_write_buf,0,REC_LEN);
	m_write_buf[ 0 ] = FIXED_FRAME_HEAD ;
...
int Iec101b::call_classtwo_data()
{
	ACE_OS::memset(m_write_buf,0,REC_LEN);
	m_write_buf[ 0 ] = FIXED_FRAME_HEAD ;
	...
	CRC_test( m_write_buf , 5 , CRC_WRITE ) ;
	m_write_buf[ 5 ] = FRAME_TAIL ;
