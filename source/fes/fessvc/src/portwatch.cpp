...
{
// 	if (!Net_stat::get_host_Mirror_flag())
// 	{
		create_host_nodes();
// 	}
// 	else		//III区前置无需其他前置的相关信息，初始化自身信息即可
// 	{
// 		int local_host_no = Net_stat::get_host_no();
// 		m_index = 0;
// 		Fe_node* node = new Fe_node(local_host_no, m_index);
...
// 	}
	...
	this->msg_queue_->high_water_mark(1024*100*16);			//调整task消息队列大小，防止putq时出现阻塞，无法插入消息
}
...
// 		tem_host_no = m_fe_nodes.at(ncheck_fep_no)->get_host_no();//获取轮询机的节点号
	...
	{
		printf("I'm the main_server of %d zone!!!\n", nlocal_collect_zone);
		for (int ncheck_fep_no = 0; ncheck_fep_no < fep_num; ncheck_fep_no++)		//轮询所有前置机的状态，并处理
		{
			int n_zone = m_fe_nodes.at(ncheck_fep_no)->get_collect_zone();
	...
	{
		printf("there's main_server in my zone,zone = %d!!!\n", nlocal_collect_zone);
	}

//todo  III区前置无需启动端口值班线程
// 	if (!Net_stat::get_host_Mirror_flag())
// 	{
		activate();
// 	}
// 	else
// 	{
// 		Proc_manager::putdbg(MAINDBG, "本机是III区前置，不启动端口值班线程，只初始化值班端口");
// 	}
	return 0;
}
...
void Port_watch::create_host_nodes(void)
{
	...
	printf("Bbbbbbbbbbbb11111::: index=%d,local_host_no=%d,node_num=%d \n",m_index,local_host_no,node_num);
	fflush(stdout);
...
// 	time(&(msg->msg_time));
	...
	{
		//新增加的通道的默认值班机设置为通道表中的默认值班机
		if (m_fes_shm.chan_data_ptr[i].curr_attend_host == -1)
		{
			m_fes_shm.chan_data_ptr[i].curr_attend_host = m_fes_shm.chan_para_ptr[i].default_attend_host;
		}		
...
// 	m_already_give_up = 0;
	...
	ACE_Time_Value tm = ACE_OS::gettimeofday() + ACE_Time_Value(0,0);
...
// 		}
		...
		if (nmsg_fep_collect_zone == nlocal_collect_zone )		//若收到放弃接管邮件，先跳出，处理
		{
	...
	for (int ncheck_fep_no = 0; ncheck_fep_no < fep_num; ncheck_fep_no++)		//轮询所有前置机的状态，并处理
	{
		...
		{	
			...
			{ 
				...
				m_fes_shm.attend_self(tem_host_no);		//设置该启动机器默认通道
				m_already_give_up = 0;
				ACE_DEBUG((LM_DEBUG, "I'm alive, so I take over my own channel\n"));
			}
...
//				m_fe_nodes.at(ncheck_fep_no)->set_load(change_bit);
				...
				ACE_DEBUG((LM_DEBUG, "My take_over fep is alive, so I give up %d fep's channel!\n", tem_host_no));
			}
		...
		else if(fep_stat == -1 && fep_load != 0)		//从运行到宕机状态前置
		{
			...
			for (int nfep_take_over_no = 0; nfep_take_over_no < fep_num; nfep_take_over_no++)				//判断本机是不是节点号最小，并确认是否接管
			{
				...
				int nfep_take_over_zone = m_fe_nodes.at(nfep_take_over_no)->get_collect_zone();
				int nfep_host_no = m_fe_nodes.at(nfep_take_over_no)->get_host_no();

				if (nfep_take_over_no == ncheck_fep_no || stat == -1 || nfep_take_over_zone != check_fep_zone) // 若是宕机的机器或非本区前置，忽略 
				{
					continue;
				}
...
// 				}
			...
			if(is_local_take)//本机接管,条件为节点号最小
			{
				if ((fep_load & local_load) == 0 )		
				{
					...
					ACE_DEBUG((LM_DEBUG, "I'm main_server of my zone, so I take over %d fep's channel! \n", take_fep_no));
				}
			}
