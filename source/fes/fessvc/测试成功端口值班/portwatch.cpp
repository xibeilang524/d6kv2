...
// 	unsigned long ori_load = 1 << m_index;		//初始化负荷状态  add by zjq
// 	m_fe_nodes.at(m_index)->set_load(ori_load);
...
void Port_watch::take_over(void)
{
	...
	for (int ncheck_fep_no = 0; ncheck_fep_no < fep_num; ncheck_fep_no++)		//轮询所有前置机的状态，并处理
	{
		...
		if (fep_stat == 0 && fep_load == 0)//从停止到运行
		{	
			if ( ncheck_fep_no == m_index && m_already_give_up )	//若是本机启动且接管其通道的前置已放弃通道，设置自身负荷
			{ 
		...
		else if(fep_stat == -1 && fep_load != 0)//从运行到停止
		{
			if (ncheck_fep_no == m_index)//如果是本机，肯定是在运行中
			{
				continue;
			}
			...
			for (int nfep_take_over_no = 0; nfep_take_over_no < fep_num; nfep_take_over_no++)				//判断本机不是负荷最小或不是节点号最小，并确认是否接管
			{
				int stat = m_fe_nodes.at(nfep_take_over_no)->get_stat();
				if (nfep_take_over_no == ncheck_fep_no || stat == -1) // 若是当机的机器或宕机的机器，忽略 
				{
					continue;
				}
