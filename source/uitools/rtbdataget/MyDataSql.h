...
typedef std::map<int,std::string>	SCDOBJ_MAP ;
...
{
public:
	RdbValueGet_B(int request_idnum);
	~RdbValueGet_B();
public:
...
private:
	...
	std::string judge_table_type( std::string obj_type ) ;

	int read_scd_obj( void ) ;
private:
	Rdb_QuickPolling* m_polling_read;
	RDB_FIELD_STRU* m_rdbFields;
	std::map<std::string , int> m_scd_obj_map ;
};
